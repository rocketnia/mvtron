...
println "running calculations on the samples it gets, culminating in a mel-"
println "frequency cepstrum. After some silence, it will output the number of"
println "audio frames, how long it took to process them, and some summaries of"
println "the calculation results."
println ""
println "Right now, the mel conversion is a bit slow."
// TODO: Either change the mel conversion to a faster form of resampling (such
// as looping through the corrected bins and setting them each to linear
// combinations of the nearest two original bins), or move it to a library.
println ""
...
reader.addListener( [
	onAudioSamples: { IAudioSamplesEvent event ->
		...
		if ( channels != 0 )
		{
			...
			//
			// We're ignoring the complex component of the FFT and ignoring
			// the negative half--since it's real numbers we're plugging in, the
			// negative half is the complex conjugate of the positive half--so
			// the first bin (the one at 0Hz) is half the size of the rest.
			double[] fftLibraryResult = sampleList;
			new DoubleFFT_1D( numSamplesUsed ).realForward( fftLibraryResult );
			...
			List binEndsInHertz = [ (double)0 ] + (1..numberOfBins).
				collect { (it - (double)0.5) * sampleRate / numSamplesUsed };
			
			// Resample the frequency spectrum by converting the frequencies to
			// mel (a logarithmic transformation of Hz meant to simulate human
			// frequency evaluation) and reorganizing them in new, equally-
			// spaced bins in preparation for sending them through another FFT.
			// Wikipedia doesn't say how big these new bins should be, so we're
			// choosing for now that the number of mel in a bin here will be
			// just the right size so that we have the same number of bins as
			// before.
			//
			// (Actually, what Wikipedia says to do is to use "triangular
			// overlapping windows" somehow. I think what that's supposed to
			// mean is that the original bins should be interpolated linearly in
			// order to determine the continuous function to re-approximate. To
			// explain a little more, each bin value's contribution in linear
			// interpolation would be shaped like a triangle, and these
			// triangles would overlap. However, this isn't exactly what we're
			// doing here. Instead of interpolating the function and taking new
			// sample values at regular intervals, we're essentially treating
			// the original bins as a zero-slope bar graph, drawing a bunch of
			// new lines on the bar graph, and measuring how much area is
			// between the new lines.)
			List binEndsInMel = binEndsInHertz.collect( hertzToMel );
			double correctedBinSizeInMel = binEndsInMel.last() / numberOfBins;
			List correctedBinEndsInMel =
				(0..numberOfBins)*.multiply( correctedBinSizeInMel );
			
			// We'll start the corrected bins at values of zero and apportion
			// each of the original bins one at a time.
			...
			numberOfBins.times {
				...
				int startPartialBinStopIndex =
					correctedBinEndsInMel.findIndexOf { binStartInMel <= it };
				...
				{
					// If this bin is wholly inside one of the corrected bins,
					// contribute its whole value to that bin.
				...
				else
				{
					...
					// corrected bins, fill that corrected bin appropriately.
					...
					// Fill all the corrected bins this bin wholly envelops.
					(startPartialBinStopIndex..<stopPartialBinStartIndex).each {
			...
			// Take the discrete cosine transform (specifically, the DCT-II) of
			// the logs of the mel-frequency powers in order to get the mel-
			// frequency cepstral coefficients (MFCCs), which make up the mel-
			// frequency cepstrum (MFC). Wikipedia doesn't say the log of the
			// /magnitude/ should be taken, but it mentions that kind of thing
			// on the cepstrum page, so we're doing that instead. However, we've
			// been ignoring the imaginary parts, so we might be skewing the
			// result somewhat by treating them as though they're zero.
			...
			// NOTE: If the corrected bin values are all zero, their logs will
			// all be the IEEE negative infinity value, and the MFCCs will all
			// be NaN. I don't know what happens if just some of them are zero,
			// but it's probably the same thing.
			
			
			// Now that the interesting raw stuff has been calculated, take
			// measurements of it.
			...
			assert !thesePeakHertzRanges.empty;
			peakHertzFrequencyHistogram[ thesePeakHertzRanges ] =
				(peakHertzFrequencyHistogram[ thesePeakHertzRanges ] ?: 0) + 1;
			...
			assert !thesePeakMelRanges.empty;
			...
			assert !thesePeakMfccIndices.empty;
...
def setCompare = { elementCompare = { a, b -> a <=> b } -> { a, b ->
	...
	{
		def potentialResult = elementCompare( sortedA[ i ], sortedB[ i ] );
...
peakHertzFrequencyHistogram.keySet().sort( setCompare( rangeCompare ) ).
	each { key ->
...
peakMelFrequencyHistogram.keySet().sort( setCompare( rangeCompare ) ).
	each { key ->
	
	println "${key.sort( rangeCompare )} -> ${peakMelFrequencyHistogram[ key ]}"
};

println ""
println "peakMfccHistogram (which should be quite trivial; this isn't a very"
println "evocative demonstration of the MFCC):"
