...
//
// Copyright 2009, 2011, 2021 Ross Angle
...
// TODO: See if this even works. It was found neglected in an old
// working copy of MVTron. It probably wasn't committed since it
// didn't do exactly what it set out to do, which was "culminating in
// a guess at the beat of the audio."
...
def handleSampleWithMappedBuffer = {
	...
	// Measure some values.
	sampleCountList.add( numSamplesPerChannel )
	meanAbsoluteGainList.add(
		samples*.abs().sum( (long)0 )
			/ (double)numSamplesPerChannel
			/ 0x10000 )
	rootMeanSquareGainList.add(
		Math.sqrt(
			samples.collect { it * it }.sum( (long)0 )
				/ (double)numSamplesPerChannel
		)
			/ 0x10000 )
	...
	// the  negative half--since it's real numbers we're plugging in,
	// the  negative half is the complex conjugate of the positive
	// half--so the  first bin (the one starting at 0Hz) is half the
	// size of the rest.
	//
	// We're also normalizing the samples to the range [-0.5,0.5) by
	// dividing by 0x10000, the number of possible 16-bit integer
	// values.
	//
	...
	List hertzBinEndsList = [ (double)0 ] +
		(1..numberOfBins).collect { (
			(it - (double)0.5) * sampleRate / numSamplesPerChannel
		) }
	...
	// Note that the following hertzToMel use is only valid because
	// the interval begins with zero Hertz, which is the same as zero
	// mel.
	double totalWidthInMel = hertzToMel( totalWidthInHertz )
	def hertzBinWidth =
		{ hertzBinEnds( it + 1 ) - hertzBinEnds( it ) }
	def melBinWidth = { melBinEnds( it + 1 ) - melBinEnds( it ) }
	
	// Measure some more values.
	// TODO: These are silly values; they treat the power at a bin as
	// though it's the power at every frequency in the bin, rather
	// than as a definite integral of the range of the frequency-power
	// function encompassing the bin. Correcting this would make
	// meanAbsoluteRealHertzPowerList and meanAbsoluteRealMelPowerList
	// proportional by a constant times sampleCountList, and a few
	// other things would probably cancel out too, but I haven't
	// thought about it much. This is high-priority.
	List hertzWeightedBins = (0..<numberOfBins).collect { (
		bins[ it ] * hertzBinWidth( it )
	) }
	meanAbsoluteRealHertzPowerList.add(
		hertzWeightedBins*.abs().sum( (double)0 )
			/ totalWidthInHertz )
	rootMeanSquareRealHertzPowerList.add(
		Math.sqrt(
			(0..<numberOfBins)
				.collect { bins[ it ] * hertzWeightedBins[ it ] }
				.sum( (double)0 )
				/ totalWidthInHertz
		) )
	...
	meanAbsoluteRealMelPowerList.add(
		melWeightedBins*.abs().sum( (double)0 )
			/ totalWidthInMel )
	rootMeanSquareRealMelPowerList.add(
		Math.sqrt(
			(0..<numberOfBins)
				.collect { bins[ it ] * melWeightedBins[ it ] }
				.sum( (double)0 )
				/ totalWidthInMel
		) )
	double binsSum = bins.sum()
	
	// This is how the expected value and variance calculations have
	// been derived (both for the Hertz scale and for the mel scale):
	//
	// E( Freq )
	...
	// ...but it doesn't save on much computation in this case (one
	// multiply less, in favor of an addition and two subtractions,
	// and a little bit less chance of catastrophic cancellation),
	// much less cancel things out, so it would probably just make
	// things more confusing than they  have to be.
	//
	// (Yeah, the above factoring could be a much simpler process if
	// the  (a - m) and (b - m) terms are never decomposed, but this
	// way the  fully expanded form is more visible in the
	// intermediate steps.)
	//
	def weightedExpectedValueAndVariance = { myBins, ends ->
		...
		double expectedValue = (
			binIndices.collect { (
				weightedBins[ it ] * (ends( it ) + ends( it + 1 ))
			) }
				.sum( (double)0 )
		) / weightedBinsSum / 2
	...
	if ( previousBins != null )
	{
		if ( previousBins.size() == numberOfBins )
		{
			...
			meanAbsoluteRealHertzDiff.add(
				(0..<numberOfBins)
					.collect { (
						diff[ it ].abs() * hertzBinWidth( it )
					) }
					.sum( (double)0 )
					/ totalWidthInHertz )
			rootMeanSquareRealHertzDiff.add(
				Math.sqrt(
					(0..<numberOfBins)
						.collect { (
							squareDiff[ it ] * hertzBinWidth( it )
						) }
						.sum( (double)0 )
						/ totalWidthInHertz ) )
			meanAbsoluteRealMelDiff.add(
				(0..<numberOfBins)
					.collect { diff[ it ].abs() * melBinWidth( it ) }
					.sum( (double)0 )
					/ totalWidthInMel )
			rootMeanSquareRealMelDiff.add(
				Math.sqrt(
					(0..<numberOfBins)
						.collect { (
							squareDiff[ it ] * melBinWidth( it )
						) }
						.sum( (double)0 )
						/ totalWidthInMel ) )
		}
		...
		{
			// Sadly, the intensity is going to have some hiccups,
			// since not  all audio frames are necessarily the same
			// size.
...
dst.addProbe( PadProbeType.BUFFER, (Pad.PROBE) { pad, info ->
	...
	if ( millisecondsBetweenCheckpoints < now - lastCheckpointTime )
	{
		...
		println "Processed ${sampleCountList.size()} audio frames " +
			"in ${(now - start)/1000} seconds..."
	}
	return PadProbeReturn.OK
} )
pipe.play()
...
println ""
println "Done! There were ${numberOfFrames} audio frames, and they " +
	"were processed in"
...
// TODO: Figure out whether sort or reverse really modifies the
// collection. That just seems weird. However, removing the "[] +"
// here messes things up.
def bestUnblurredDistances =
	([] + unblurredDistances).sort().reverse()

if ( numberOfFrames != 0 )
{
	bestUnblurredDistances
		.findAll { bestUnblurredDistances.head() / 2 <= it }
		.each { distance ->
			unblurredDistances.findIndexValues { it == distance }
				.each {
					println "Frame $it has unblurred distance " +
						"$distance."
				}
		}
}
...
println ""
// TODO: See if we can do something about displaying the data in
// Eclipse.
println "(In Eclipse, there might not be anything showing here because long"
println "lines aren't displayed.)"
println ""
